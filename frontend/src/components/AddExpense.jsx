  ...
  const handleSubmit = async (e) => {
    try {
      ...
      const res = await axios.get(
        `${process.env.REACT_APP_BACKEND_URL}/api/v1/get-total/${id}/${month}`
      );
  
      const totalIn = res.data.totalIn;
      const totalOut = res.data.totalOut;
  
      if (totalOut + outVal > 0.8 * (totalIn + inVal)) {
        alert("Sorry!! Your expenses are exceedding 80% of your income");
        return;
      } else {
        await axios.post(
          `${process.env.REACT_APP_BACKEND_URL}/api/v1/add-data/${id}`,
          {
            month,
            date,
            inVal,
